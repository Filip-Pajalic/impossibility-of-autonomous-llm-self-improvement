...
\begin{assumption}[AGI Target Nontriviality]
\label{ass:agi_nontrivial}
The AGI criterion is interpreted over a specified family of task distributions $\mathcal{D}_f$, input encodings, and resource bounds. This assumption is necessary to avoid making the target impossible for every finite physical system by definition. A complete formalization must state these restrictions explicitly and prove that the target remains nontrivial.
\end{assumption}
...
\begin{theorem}[Conditional Information Ceiling]
\label{thm:info_ceiling}
Let $p_{\mathrm{true}}$ denote the ground-truth distribution over all valid reasoning chains. Suppose the variables representing ground truth, training data, the initial model, self-generated data, and later models form the Markov chain used below. For the autonomous self-improvement sequence $\{\bm{\theta}_k\}$:
\begin{equation}
...
\begin{proof}
The self-generated dataset $\mathcal{D}_k^{\mathrm{self}}$ is produced by sampling from $p_{\bm{\theta}_k}$. Under the Markov-chain assumption:
\begin{equation}
...
The information ceiling is a statement about mutual information under the stated Markov model. It does not by itself rule out better extraction or use of information already present in the model. A complete proof must therefore distinguish information creation from computational access to latent information.
\end{remark}
...
\begin{theorem}[Conditional Entropy Contraction Under Self-Distillation]
\label{thm:collapse}
Let $p_{\bm{\theta}_k}$ be the model at step $k$, and let $p_{\bm{\theta}_{k+1}}$ be obtained by training on $N$ samples from $p_{\bm{\theta}_k}$ with no external filtering, selection, smoothing, entropy regularization, or fresh external data. Under this finite-sample empirical self-distillation regime:
\begin{equation}
H(p_{\bm{\theta}_{k+1}}) \leq H(p_{\bm{\theta}_k})
\end{equation}
with strict inequality requiring the stated finite-sample and training assumptions. The empirical support contracts monotonically:
\begin{equation}
...
\end{enumerate}
Under the resource-scaling assumptions above, every attempt to provide the system with sufficient resources for self-correction generates a new, larger resource requirement. The gap between what the system needs and what it has is not a fixed deficit to be closed by engineering---it is a structural property that scales with any attempt to close it. Combined with the conditional error model of Theorem~\ref{thm:error_compound}, this creates an anti-convergent loop rather than a guaranteed path to self-correction.
...
\begin{theorem}[Conditional Error Divergence Without External Correction]
\label{thm:error_compound}
Let $\epsilon_k$ measure the error of $\bm{\theta}_k$ relative to ground truth. Assume the recursive self-training process satisfies:
\begin{equation}
\epsilon_{k+1} = \epsilon_k - \Delta_k + \delta_k + \sum_{j=0}^{k} \alpha_{k,j}\, \delta_j
\end{equation}
where $\Delta_k \geq 0$ is the intended improvement, $\delta_k > 0$ is undetected degradation, and $\alpha_{k,j} \geq 0$ captures error propagation through self-generated training data.

If errors propagate through the data chain, $\Delta_k \to 0$, and $\delta_k$ remains bounded away from zero, then:
\begin{equation}
\liminf_{k \to \infty} \epsilon_k = \infty
\end{equation}
The error diverges under these assumptions. Without these assumptions, the theorem should be read as a recurrence analysis rather than a universal claim about every possible autonomous process.
\end{theorem}
...
\label{thm:main}
Let $\{p_{\bm{\theta}_k}\}_{k=0}^{\infty}$ be the sequence of models produced by autonomous self-improvement (Definition~\ref{def:selfimprove}). Assume the AGI nontriviality condition (Assumption~\ref{ass:agi_nontrivial}) and the eight barriers listed below hold under the formal definitions of the model, training process, and AGI criterion. Then $\{p_{\bm{\theta}_k}\}$ cannot converge to AGI (Definition~\ref{def:agi}) unless the initial model already satisfies the AGI criterion.

\begin{enumerate}
    \item \textbf{Information ceiling} (Theorem~\ref{thm:info_ceiling}): Under the Markov-chain assumptions, self-generated data cannot create new mutual information about ground truth. Inference-time search can better utilize existing information, so the proof must distinguish information creation from computational extraction.
    
    \item \textbf{Distributional collapse} (Theorem~\ref{thm:collapse}): Under finite-sample, unsmoothed, unfiltered empirical self-distillation, self-training contracts empirical support and can reduce entropy. Mechanisms that prevent collapse must be analyzed for whether they introduce external information.
    
    \item \textbf{G\"{o}delian verification failure} (Theorem~\ref{thm:godel}): The model cannot completely verify its self-modifications as genuine global improvements. Partial verification (benchmarks, test cases) is possible. The human-brain asymmetry discussion is empirical background rather than a formal premise.
    ...
    \item \textbf{Pattern completion barrier} (Theorem~\ref{thm:separation}): The target claim is that stochastic pattern completion on finite training data is weaker than general computation under precise finite-model assumptions. Compositionality enables recombination but remains bounded by the information ceiling. Practical transformers are finite automata, not Turing machines.
    
    \item \textbf{Computational complexity barrier} (Theorem~\ref{thm:pnp}, Corollary~\ref{cor:pnp_info}, and Remark~\ref{rem:pnp_empirical}): If the relevant self-improvement search problem instantiates an NP-hard optimization family and $\mathrm{P} \neq \mathrm{NP}$, then the gap between verification and search is permanent. Current LLM failures on NP-hard benchmarks are empirical context, not proof of $\mathrm{P} \neq \mathrm{NP}$.
    ...
    \item \textbf{Error divergence} (Theorem~\ref{thm:error_compound}): Under the stated recurrence, lower-bound, propagation, and vanishing-improvement assumptions, statistical errors compound through recursive self-training. The universality of those assumptions remains a proof obligation.
\end{enumerate}
...
H.~Zenil.
\newblock On the limits of self-improving in large language models: The singularity is not near without symbolic model synthesis.
\newblock \emph{arXiv preprint arXiv:2601.05280}, 2026.
