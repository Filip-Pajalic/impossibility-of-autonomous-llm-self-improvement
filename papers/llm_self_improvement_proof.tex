...
\begin{enumerate}
    \item \textbf{The sealed loop does not produce open-ended improvement.} No reviewer produced a counterexample: a mechanism by which a text-only autoregressive model, consuming no externally grounded signal and replacing rather than accumulating its data, reaches the AGI criterion. The independent formal treatments agree---Zenil~\cite{limitsself2025} derives entropy decay and variance amplification under a vanishing exogenous signal; the model-collapse literature~\cite{shumailov2024model,dohmatob2024strong,seddik2024synthetic} derives degradation under replacement; the sharpening analysis~\cite{huang2024sharpening} bounds self-improvement by the base policy's coverage; and Yue et al.~\cite{yue2025rlvr} find RLVR reasoning bounded by the base model at large $k$. The conditional core of Section~\ref{sec:info} is the formal statement of this consensus, and it is now machine-checked.
    \item \textbf{Improvement tracks the strength of the external signal.} Chen et al.~\cite{chen2026rsi} report a verification hierarchy---formal verifiers strongest, intrinsic self-assessment weakest---with demonstrated improvement tracking it. Our grounding audit (\S\ref{sec:grounding_audit}) finds the same ordering: execution-checked systems~\cite{novikov2025alphaevolve,zhao2025azr} and test-suite-checked systems~\cite{zhang2025dgm,zhang2026selfharness} report large gains; the two systems with no external correctness signal~\cite{huang2025rzero,pan2026coverrl} report single-digit gains and exhibit the predicted diversity collapse.
    \item \textbf{Grounding saturates at the grounder's competence.} Yi et al.~\cite{yi2026escaping} prove that verifier-guided retraining converges to the verifier's knowledge center, with gains that plateau or reverse as the verifier is imperfect. This is the strongest quantitative form of the thesis available and it is not ours; it converges with our conclusion from a different direction.
    \item \textbf{Unaided self-verification is unreliable.} Agreed by every reviewer and by the empirical record, with the qualification in item~3 of \S\ref{sec:contested}.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Does RL expand the base model's reasoning boundary?} Yue et al.~\cite{yue2025rlvr} say no---RLVR sharpens what the base model can already sample. Liu et al.~\cite{liu2025prorl} say yes---prolonged RL finds solutions the base model fails at every $k$. This is the empirical crux for how much practical bite Corollary~\ref{cor:accuracy_ceiling} has, and it is unresolved. We do not claim it in our favour.
    \item \textbf{Is collapse a property of self-training or of a training recipe?} Gerstgrasser et al.~\cite{gerstgrasser2024accumulate} bound the error under accumulation; Kang et al.~\cite{kang2025demystifying} find no degradation from rephrased synthetic data at realistic mixing ratios while textbook-style synthesis does collapse. The consensus that survives is narrower than ``self-training collapses'': collapse is a function of the regime, the synthesis method, and the mixing ratio.
    \item \textbf{How much of the self-correction failure is real?} Chen et al.~\cite{chen2026selfcorrection} show that relabeling an error's apparent source, content held fixed, moves explicit correction rates by 23--93 points. The 2023 result stands as evidence about presentation-sensitive behaviour, not about a hard capability ceiling.
\end{enumerate}

\subsection{Rejected or withdrawn}

Consensus was \emph{against} several claims made in earlier versions of this manuscript, and they have been withdrawn or demoted rather than defended:

\begin{itemize}
    \item The unrestricted AGI criterion (unsatisfiable; Proposition~\ref{prop:vacuity}).
    \item Strict information decrease under the data processing inequality (Remark~\ref{rem:no_strictness}).
    \item Literal support contraction (vacuous for softmax models; Section~\ref{sec:collapse_note}).
    \item Universal error divergence (false in the accumulation regime; Section~\ref{sec:regimes}).
    \item Parameter count growing with context length (empirically false; Remark~\ref{rem:rope}).
    \item ``Human influence is mathematically necessary'' as a general claim---replaced by external grounding, with the deduction and embodiment boundary cases conceded (\S\ref{sec:grounding_discussion}).
...
Reviewers converge on one meta-observation, which we accept: the strength of the headline claim came from the breadth of the definitions rather than from the mathematics. Stated at the width the mathematics supports, the residual claim is:

\begin{quote}
A sealed autoregressive self-distillation loop gains no information about ground truth, and its achievable accuracy on a fixed task family is capped by the information its initial model already carries. Every demonstrated route past that cap consumes an external grounding signal, and the improvement it buys appears to saturate at the competence of that signal.
\end{quote}

The first sentence is proved (Theorem~\ref{thm:info_ceiling}, Corollary~\ref{cor:accuracy_ceiling}, machine-checked). The second is an empirical regularity with one formal result behind it~\cite{yi2026escaping} and no counterexample we are aware of. The quantitative version---capability per bit of grounding---is the open problem, and it is a better problem than the one this manuscript started with.
