...
\title{\textbf{On the Impossibility of Autonomous Self-Improvement\\to General Intelligence in Stochastic Language Models:\\A Proof Program via Information Theory, G\"{o}delian Limits,\\and Distributional Collapse}\\[10pt]\large Revised Edition with Counterargument Analysis}
...
\begin{abstract}
We develop a proof program for the claim that large language models (LLMs) based on stochastic next-token prediction cannot, through autonomous self-improvement without external human influence, converge to artificial general intelligence (AGI). We model the LLM as a conditional probability estimator trained via maximum likelihood on a fixed distributional support and identify independent barriers requiring formal verification: (1) the information-theoretic ceiling imposed by training data entropy; (2) distributional collapse under recursive self-training; (3) G\"{o}delian and computability bounds on self-verification; (4) the stochastic gradient descent fixed-point trap; (5) the gap between statistical pattern completion and general reasoning; (6) conditional computational-complexity barriers; (7) context and computation resource constraints; and (8) error compounding under recursive self-modification. We address counterarguments regarding inference-time search, reinforcement learning with self-play, tool-augmented systems, and multi-agent architectures, arguing that each either falls within the proof's scope or introduces external information that should be treated as human influence under the definitions below. We further provide a control theory perspective and empirical corroboration from model-collapse, self-verification, and code-maintenance benchmarks. The current Lean formalization is a work in progress: several bridge assumptions and theorem obligations remain open, so the conclusion should be read as a conditional proof program rather than a completed machine-checked theorem.
\end{abstract}
...
This manuscript is accompanied by a Lean~4 formalization skeleton. The Lean project currently typechecks, but it is not yet a completed proof: several central results are represented by proof obligations, bridge axioms, or placeholders. The claims below should therefore be read in three categories:
...
\end{enumerate}
The verification plan for closing these gaps is maintained separately in \texttt{docs/verification\_plan.md}.
\end{remark}
...
\label{thm:main}
Let $\{p_{\bm{\theta}_k}\}_{k=0}^{\infty}$ be the sequence of models produced by autonomous self-improvement (Definition~\ref{def:selfimprove}). Assume the eight barriers listed below hold under the formal definitions of the model, training process, and AGI criterion. Then $\{p_{\bm{\theta}_k}\}$ cannot converge to AGI (Definition~\ref{def:agi}) unless the initial model already satisfies the AGI criterion.
...
\noindent Under these assumptions, the target conclusion is:
\begin{equation}
\boxed{\forall\; \bm{\theta}_0 \in \mathbb{R}^p,\;\; \forall\; k \geq 0: \quad \mathcal{S}^k(\bm{\theta}_0) \not\to \bm{\theta}^*_{\mathrm{AGI}}}
\end{equation}
within the autonomous stochastic autoregressive framework, excluding the degenerate case in which $\bm{\theta}_0$ already has AGI-level information. The remaining work is to discharge the assumptions above in a machine-checkable form.
\end{theorem}
